...
void Worker::stop() {
    ...
    //停止收取任务结果线程
    if(receive_task_result_thread_.joinable())
        receive_task_result_thread_.join();
...
void Worker::report_task_result() {
    ...
    while(running_)
    {
        ...
        if(!task_result_queue_.empty())
        {
            ...
            //上报任务结果
            worker_result_queue_->publishTaskResult(task_result);
        }
...
void Worker::report_heartbeat() {
    ...
    while(running_)
    {
        ...
        heartbeat.SerializeToString(&heartbeat_data);
        //修改zk节点数据
        zkcli_->updateNode(WORKER_ZK_PATH + "/" + WORKER_ID, heartbeat_data);
        //睡眠10秒
...
void Worker::exec_task() {
    ...
    while(running_)
    {
        ...
        if(!pending_tasks_.empty())
        {
            ...
            //任务上下文
            std::string context = task.context();
            //任务ID
            ...
            if(task_type == "COMMAND"){
                //组装参数
                std::string command = task.context();
                ...
                if(result != "")
                {
                    ...
                    task_result.set_error_code(0);
                }
                else
                {
                    task_result.set_error_message("命令执行失败");
                    task_result.set_error_code(1);
                }
                
            }
            else if(task_type == "FUNCTION"){
                //执行函数
            ...
            task_result.set_timestamp(time(nullptr));
